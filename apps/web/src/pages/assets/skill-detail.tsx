...
import { getSkillReferences, type SkillAction } from '../../skill-installation'
...
export function SkillDetail({ asset }: { asset: FullAsset }) {
  ...
  return <><PageHeader backTo="/assets/skills" backLabel="返回 Skills" title={asset.name} description={`${asset.summary} · 本体安装与 Agent 引用相互独立`} action={<div className="flex flex-wrap gap-2">{skill.installation.status === 'available' && <Button onClick={() => setAction('install')}>模拟安装</Button>}{skill.installation.status === 'update-available' && <Button onClick={() => setAction('update')}>模拟更新</Button>}{skill.installation.status !== 'available' && skill.installation.previousVersions.length > 0 && <Button variant="outline" onClick={() => setAction('rollback')}>模拟回滚</Button>}{skill.installation.status !== 'available' && <Button variant="outline" onClick={() => setAction('uninstall')}>模拟卸载</Button>}</div>} /><div className="grid gap-5 lg:grid-cols-[1fr_360px]"><section className="panel p-5"><FieldRow label="来源">{skill.source.kind === 'local' ? `本地 · ${skill.source.path}` : skill.source.kind === 'git' ? `Git · ${skill.source.repository}@${skill.source.ref}` : `${skill.source.provider} · Marketplace 预置 Mock`}</FieldRow><FieldRow label="交付方式">{skill.delivery.kind === 'plugin' ? `Plugin 提供 · ${skill.delivery.pluginAssetId}` : '独立 Skill'}</FieldRow><FieldRow label="安装状态"><StatusBadge tone={toneForStatus(asset.status)}>{asset.status}</StatusBadge></FieldRow><FieldRow label="版本">当前 {skill.installation.installedVersion ?? '未安装'} · 可用 {skill.installation.availableVersion}</FieldRow><FieldRow label="历史版本">{skill.installation.previousVersions.join('、') || '无'}</FieldRow><FieldRow label="声明权限">{skill.review.permissions.join('；')}</FieldRow><FieldRow label="影响">{skill.review.impact.join('；')}</FieldRow><FieldRow label="文件">{skill.review.files.join('；')}</FieldRow><FieldRow label="演示路径"><MonoPath>{asset.path}</MonoPath></FieldRow></section><aside className="panel overflow-hidden"><div className="border-b border-border p-4"><b>显式引用</b><p className="mt-1 text-xs text-muted-foreground">安装动作不会自动修改以下关系。</p></div>{refs.length ? <div className="divide-y divide-border">{refs.map((ref, index) => <Link key={`${ref.agentId}-${ref.workspaceId ?? index}`} to={`/agents/${ref.agentId}?tab=skills`} className="block p-4 hover:bg-muted"><b>{ref.agentName}</b><small className="mt-1 block text-muted-foreground">{ref.workspaceId ? `WorkspaceBinding · ${ref.workspaceId}` : 'Agent 根级引用'}</small></Link>)}</div> : <p className="p-4 text-sm text-muted-foreground">暂无显式引用。</p>}</aside></div><div className="mt-5"><MockBoundaryNote>本页不会下载、写入、删除文件或执行安装脚本；所有状态刷新后恢复初始 Mock。</MockBoundaryNote></div>{action && <SkillActionDialog asset={asset} action={action} onClose={() => setAction(undefined)} />}</>
}
