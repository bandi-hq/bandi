  ...
  return <>
    ...
    <div className={`grid gap-5 ${desktop ? '' : 'xl:grid-cols-[1.35fr_.65fr]'}`}>
      ...
      <section id="pending-config" className="panel scroll-mt-24 border-l-[3px] border-l-warning p-5"><div className="flex items-center justify-between"><div className="label">待处理</div><StatusBadge tone={issues ? 'warning' : 'success'}>{issues} 项</StatusBadge></div>{issues ? <div className="mt-4 space-y-4">
        ...
        {state.agentDiagnostics.map((diagnostic, index) => <div key={`${diagnostic.source}-${diagnostic.code}-${diagnostic.path}-${index}`} className="flex items-start gap-3"><CircleAlert size={18} className="mt-0.5 shrink-0 text-danger" aria-hidden="true" /><span className="min-w-0 flex-1"><b className="block text-sm">Agent 配置需要处理</b><small className="block text-muted-foreground">{diagnostic.message}</small>{diagnostic.remediation && <small className="block text-muted-foreground">{diagnostic.remediation}</small>}<details className="mt-1 text-xs text-muted-foreground"><summary className="cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">技术详情</summary><span className="mt-1 block break-words font-mono [overflow-wrap:anywhere]">{diagnostic.code}{diagnostic.source ? ` · ${diagnostic.source}` : ''}{diagnostic.path ? ` · ${diagnostic.path}` : ''}</span></details></span><Button variant="outline" size="sm" onClick={hydrateDesktop}><RefreshCw size={14} aria-hidden="true" />重新读取</Button></div>)}
        {recoveries.map((operation) => {
          const deleting = operation.operationKind === 'delete'
          return <div key={operation.id} className="flex items-start gap-3"><CircleAlert size={18} className="mt-0.5 shrink-0 text-warning" aria-hidden="true" /><span className="min-w-0 flex-1"><b className="block text-sm">{deleting ? 'Agent 删除清理尚未完成' : 'Agent 配置尚未完整保存'}</b><small className="block text-muted-foreground">{state.agents.find((item) => item.id === operation.agentId)?.name ?? operation.agentId} · {operation.status === 'blocked' ? '内容已变化，不会自动覆盖' : deleting ? 'Agent 配置已删除，请勿重复删除' : '可安全继续未完成阶段'}</small></span>{operation.status === 'blocked' ? <Button asChild variant="outline" size="sm"><Link to={`/agents/${operation.agentId}`}>查看 Agent</Link></Button> : <Button variant="outline" size="sm" disabled={Boolean(recovering)} aria-busy={recovering === operation.id} onClick={() => void recover(operation.id)}>{recovering === operation.id ? (deleting ? '清理中…' : '修复中…') : (deleting ? '继续清理' : '继续修复')}</Button>}</div>
        })}
