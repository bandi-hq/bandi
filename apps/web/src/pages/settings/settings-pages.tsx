...
function StorageLocationPanel({ value, savedValue, onChange, onReset, onSave }: { value: string; savedValue: string; onChange: (value: string) => void; onReset: () => void; onSave: () => void }) {
  ...
  return <div className="space-y-5"><Panel title="存储位置"><label className="text-sm font-medium">Agent 根目录<input className="mt-2 h-10 w-full px-3" value={value} onChange={(event) => onChange(event.target.value)} /></label><p className="mt-3 text-xs leading-5 text-muted-foreground">所有 AgentPackage 的统一存放位置。每个 Agent 使用稳定 ID 的独立目录，不随部门或工作区变化。</p><p className="mt-2 text-xs leading-5 text-muted-foreground">保存只更新当前页面内存，不创建、移动或扫描真实目录。</p><div className="mt-5 flex justify-end gap-2"><Button variant="outline" disabled={!dirty} onClick={onReset}>取消</Button><Button disabled={!dirty} onClick={onSave}>保存演示设置</Button></div></Panel><Panel title="外部变化保护"><Select label="演示检查频率" value={state.settings.externalChangeInterval} values={['手动', '5 分钟', '15 分钟']} onChange={(next) => dispatch({ type: 'UPDATE_SETTINGS', changes: { externalChangeInterval: next as typeof state.settings.externalChangeInterval } })} /><p className="mt-3 text-xs leading-5 text-muted-foreground">多个终端或编辑器的并发修改统一表现为相对编辑基线的外部变化，并通过 Diff 处理；Bandi 不识别或展示终端与会话状态。Web mock 不会执行扫描。</p></Panel></div>
}
