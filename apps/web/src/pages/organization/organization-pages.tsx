    ...
    <div className="grid min-w-0 items-start gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="space-y-4">
        <section className="panel p-5">
          ...
          <div className="mt-4 space-y-2">{visibleMembers.map(memberRow)}{!members.length && <p className="text-sm text-muted-foreground">暂无长期 Agent。</p>}{members.length > visibleMembers.length && <Button className="w-full" variant="outline" onClick={() => setMembersDialogOpen(true)}>查看全部 {members.length} 位成员</Button>}</div>
        </section>
