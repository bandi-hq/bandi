...
import { createManagedAgent, isDesktopRuntime } from '../../desktop-bridge'
...
export function AgentCreatePage() {
  ...
  const submit = async () => {
    ...
    if (!identityValid || !dutiesValid || saving) return
    ...
    try {
      if (desktop && !importMode) {
        ...
        const result = await createManagedAgent(agent, files, avatar)
        dispatch({
    ...
    <div className="mx-auto max-w-4xl panel overflow-hidden">
      ...
      <div className="min-h-[420px] p-6 max-sm:p-4">
        {step === 1 && <div className="grid gap-5 sm:grid-cols-2">
          ...
          <SelectField label="Role" value={roleId} onChange={setRoleId} options={roles.map((item) => [item.id, item.name])} error={submitted && !selectedRole ? '请选择适用于当前公司和部门的有效 Role。' : undefined} />
