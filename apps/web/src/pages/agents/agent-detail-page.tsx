...
function IdentityTab({ agent }: { agent: FullAgent }) {
  ...
  const saveLifecycle = async () => {
    ...
    try {
      ...
      else if (commit.operation.status !== 'completed' || !commit.agent) { setSaveError(commit.operation.status === 'blocked' ? 'AgentPackage 内容已发生变化，系统未自动覆盖；请从首页待处理项查看。' : '生命周期与组织关系尚未完整保存，可从首页待处理项继续修复。') }
  ...
  return <><section className="panel overflow-hidden"><TabHeader title="身份与职责" description="组织身份、职责和服务授权不形成隐式权限。" editing={editing} onEdit={beginEdit} onCancel={cancel} onSave={save} saveLabel={managedAvatar ? '保存' : '保存到当前页面'} />
...
function InstructionsTab({ agent }: { agent: FullAgent }) {
  ...
  const save = async () => {
    ...
    if (!editor) { setError('缺少服务签发的主指令基线，请重新加载编辑器。'); return }
    ...
    try {
      ...
      if (result.kind === 'saved' || result.kind === 'unchanged') {
        dispatch({ type: 'UPSERT_MANAGED_AGENT', agent: { ...agent, instructions: text }, message: result.kind === 'saved' ? '主指令已保存到 AgentPackage' : '主指令无变化' })
        setCanonical(text); setEditing(false); setEditor(undefined); setRecoveryRef(undefined)
      } else if (result.kind === 'baseline_changed') {
        setConflict(result); setError('主指令已在编辑期间发生变化。请比较原始、当前和拟议内容后重新编辑。')
      } else {
...
function MemoryTab({ agent }: { agent: FullAgent }) {
  ...
  const scopeLabel = (space: MemorySpaceDto | (typeof demoSpaces)[number]) => {
    ...
    return { agent_long_term: 'Agent 长期记忆', agent_workspace: 'Agent 工作区记忆', workspace_shared: '工作区公共记忆', department_workspace: '部门工作区记忆' }[(space as MemorySpaceDto).scopeType]
  }
...
function WorkspacesTab({ agent }: { agent: FullAgent }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (result.kind === 'saved' || result.kind === 'unchanged') { commit(draft, result.kind === 'saved' ? '工作区配置已保存到 AgentPackage' : '工作区配置无变化'); reset() }
