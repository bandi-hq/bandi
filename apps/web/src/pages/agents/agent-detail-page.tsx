...
import { ArrowLeft, Copy, FileDiff, History, KeyRound, Plus, Save, Search, ShieldCheck, Trash2 } from 'lucide-react'
...
import { applyAgentConfig, mergeContextPolicy, normalizeAgentName, parseAgentComponentRefs, parseAgentContextConfig, parseAgentMcpRefs, parseAgentOrchestrationPolicy, parseAgentPermissions, parseAgentRuleRefs, parseAgentSkillRefs, parseAgentSopRefs, parseWorkspaceBindingConfig, serializeAgentConfig, validateAgentName, validateContextPolicy, validateContextWindowTokens, validateWorkspaceBindingConfig, type AgentContextConfig, type AgentIdentityConfig } from '../../agent-config-model'
...
import { commitManagedAgentIdentity, createMemoryCandidate, createWorkspaceBinding, discoverConfig, discoverEligibleMemorySpaces, isDesktopRuntime, listConfigRevisions, listMemoryReviews, loadConfigEditor, loadManagedAgentIdentity, readConfigRevisionContent, recoverConfigRevision, recoverManagedAgentIdentity, restoreConfigRevision, restoreManagedAgentIdentity, saveConfig } from '../../desktop-bridge'
...
export function AgentDetailPage() {
  ...
  const roleName = agent.roleId
    ...
    : '未关联组织'
  const identitySummary = [roleName, agent.department].filter(Boolean).join(' · ')
...
function ReadOnlyAgentConfig({ reason }: { reason: string }) {
  return <section className="panel p-5"><StatusBadge tone="warning">只读</StatusBadge><h3 className="mt-4 font-semibold">当前 Agent 配置不可编辑</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{reason} 可切换到 原始文件视图查看已登记的配置内容。</p></section>
}
...
function IdentityTab({ agent }: { agent: FullAgent }) {
  ...
  const saveLifecycle = async () => {
    ...
    try {
      ...
      const commit = await commitManagedAgentIdentity(lifecycleRequestId.current, applied, manifest, loaded.baselineRef, loaded.canonicalContent, agent.serviceGrants, { kind: 'keep' })
      ...
      else if (commit.operation.status !== 'completed' || !commit.agent) { setSaveError(commit.operation.status === 'blocked' ? 'Agent 配置内容已发生变化，系统未自动覆盖；请从首页待处理项查看。' : '生命周期与组织关系尚未完整保存，可从首页待处理项继续修复。') }
      else { dispatch({ type: 'UPSERT_MANAGED_AGENT', agent: { ...commit.agent, serviceGrants: agent.serviceGrants }, message: lifecycleTarget === 'archived' ? 'Agent 已归档' : lifecycleTarget === 'inactive' ? 'Agent 已停用' : 'Agent 已重新启用' }); setLifecycleTarget(undefined); lifecycleRequestId.current = undefined }
    } catch (error) { setSaveError(error instanceof Error ? error.message : String(error)) }
    ...
    try {
      ...
      if (commit.operation.status !== 'completed' || !commit.agent) {
        setSaveError(commit.operation.status === 'blocked'
          ? 'Agent 配置内容已发生变化，系统未自动覆盖；请从首页待处理项查看。'
          : '身份与组织关系尚未完整保存，可从首页待处理项继续修复。')
        return
      }
      dispatch({ type: 'UPSERT_MANAGED_AGENT', agent: { ...commit.agent, serviceGrants }, message: '身份、职责、部门成员与服务授权已保存' })
  ...
  return <><section className="panel overflow-hidden"><TabHeader title="身份与职责" description="组织身份、职责和服务授权不形成隐式权限。" editing={editing} onEdit={beginEdit} onCancel={cancel} onSave={save} canSave={!nameError} saveLabel={managedAvatar ? '保存' : '保存到当前页面'} />
    ...
    </div></section><AppDialog open={Boolean(lifecycleTarget)} onOpenChange={(open) => { if (!open) setLifecycleTarget(undefined) }} title={lifecycleTarget === 'archived' ? '归档 Agent' : lifecycleTarget === 'inactive' ? '停用 Agent' : '重新启用 Agent'} description="使用状态变更会保存到完整的 Agent 主配置文件。" footer={<><Button variant="outline" onClick={() => setLifecycleTarget(undefined)}>取消</Button><Button variant={lifecycleTarget === 'archived' ? 'danger' : 'default'} disabled={lifecycleSaving} onClick={saveLifecycle}>{lifecycleSaving ? '保存中…' : '确认更新'}</Button></>}><div className="rounded-lg border border-border bg-muted/35 p-4 text-sm leading-6"><b>保留与影响</b><p className="mt-2 text-muted-foreground">Agent 配置、{agent.workspaceBindings.length} 项工作区配置、正式记忆和配置版本全部保留。停用或归档后不可接受新委派；已有静态引用不会自动删除。</p></div></AppDialog><AppDialog open={historyOpen} onOpenChange={(open) => { setHistoryOpen(open); if (!open) { setSelectedRevision(undefined); setRestoreConfirmed(false) } }} title="身份与职责版本历史" description="历史版本不可变；恢复仅写入完整的 Agent 主配置文件，并生成新的配置版本。头像文件不在历史中。" size="xl" footer={<><Button variant="outline" onClick={() => setHistoryOpen(false)}>关闭</Button><Button disabled={!selectedRevision || selectedContent === identityEditor?.canonicalContent || !restoreConfirmed || historyLoading} onClick={restoreIdentityRevision}>恢复为新版本</Button></>}>
...
function MemoryTab({ agent }: { agent: FullAgent }) {
  ...
  const scopeLabel = (space: MemorySpaceDto | (typeof demoSpaces)[number]) => {
    ...
    return memoryScopeLabel((space as MemorySpaceDto).scopeType)
  }
...
function PermissionsTab({ agent }: { agent: FullAgent }) {
  ...
  return <><div className="grid gap-5 lg:grid-cols-2"><section className="panel p-5"><div className="flex items-center gap-2"><ShieldCheck className="text-success" aria-hidden="true" /><b>当前有效边界</b></div><FieldRow label="文件写入">{canonical.files}</FieldRow><FieldRow label="命令">{canonical.commands}</FieldRow><FieldRow label="网络">{canonical.network}</FieldRow><FieldRow label="委派">{canonical.delegation}</FieldRow></section><section className="panel p-5"><div className="label">边界调整</div><p className="mt-3 text-sm leading-6 text-muted-foreground">{desktopManaged ? '收紧可直接保存；扩大长期权限时，本地服务会要求一次性确认。' : '当前浏览器演示只保存到当前页面；扩大到工作区外仍需独立确认。'}</p>{editing ? <><label className="mt-5 block text-sm font-medium">文件写入<select value={draft.files} onChange={(event) => setDraft((value) => ({ ...value, files: event.target.value }))} className="mt-2 h-10 w-full px-3"><option>未授予</option><option>只读当前工作区</option><option>仅当前工作区</option><option>任意目录</option></select></label><div className="mt-4 flex gap-2"><Button variant="outline" disabled={busy} onClick={reset}>取消</Button><Button disabled={!dirty || busy} onClick={save}>{busy ? '保存中…' : desktopManaged ? '保存边界' : '保存到当前页面'}</Button></div></> : <Button className="mt-5" variant="outline" disabled={busy} onClick={beginEditing}><KeyRound size={16} aria-hidden="true" />{busy ? '加载中…' : '调整权限'}</Button>}</section></div><div className="mt-4 flex justify-end"><Button variant="ghost" size="sm" disabled={busy || editing} onClick={openHistory}><History size={14} aria-hidden="true" />版本历史</Button></div>{error && <div role="alert" className="mt-5 rounded-lg border border-danger/30 bg-danger/5 p-4 text-sm text-danger"><p>{error}</p>{recoveryRef && <Button className="mt-3" variant="outline" size="sm" disabled={busy} onClick={recoverRevision}>补记配置版本</Button>}</div>}{conflict && <div className="mt-5 panel p-4"><div className="grid gap-3 lg:grid-cols-3" aria-label="长期权限外部变化比较">{([{ label: '开始编辑时', side: conflict.base }, { label: '磁盘当前内容', side: conflict.current }, { label: '你的拟议内容', side: conflict.proposed }] as const).map(({ label, side }) => <section key={label} className="min-w-0 rounded-lg border border-border p-3"><b className="text-xs">{label}</b><pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap break-words text-xs leading-5">{side.content}</pre></section>)}</div><div className="mt-3 flex justify-end"><Button variant="outline" size="sm" disabled={busy} onClick={reloadConflict}>加载磁盘当前内容</Button></div></div>}<AppDialog open={confirmOpen} onOpenChange={(open) => { setConfirmOpen(open); if (!open) { setConfirmName(''); setUnderstood(false) } }} title="确认扩大 Agent 长期权限" description={desktopManaged ? '本次确认只适用于当前配置和本次修改；保存后立即失效。' : '仅在当前页面更新，不会写入真实配置。'} footer={<><Button variant="outline" disabled={busy} onClick={() => setConfirmOpen(false)}>返回编辑</Button><Button variant="danger" disabled={!confirmed || busy || (desktopManaged && !challengeRef)} onClick={confirm}>{busy ? '保存中…' : '确认扩大权限'}</Button></>}><div className="rounded-lg border border-danger/30 bg-danger/5 p-4 text-sm leading-6"><b>影响范围</b><p className="mt-2 text-muted-foreground">Agent：{agent.name}<br />文件写入：{canonical.files} → {draft.files}<br />{desktopManaged ? '只修改长期配置，不表示当前会话或进程权限已变化。' : '仅更新当前页面，刷新后恢复初始状态。'}</p></div><label className="mt-5 block text-sm font-medium" htmlFor="permission-confirm-name">输入 Agent 名称“{agent.name}”确认<input id="permission-confirm-name" className="mt-2 h-10 w-full px-3" value={confirmName} onChange={(event) => setConfirmName(event.target.value)} autoComplete="off" /></label><label className="mt-4 flex items-start gap-3 text-sm leading-6"><input className="mt-1" type="checkbox" checked={understood} onChange={(event) => setUnderstood(event.target.checked)} /><span>我理解这是长期权限扩大，不是普通配置更新，也不会自动影响当前运行期。</span></label></AppDialog><AppDialog open={historyOpen} onOpenChange={(open) => { setHistoryOpen(open); if (!open) { setSelectedRevision(undefined); setRestoreConfirmed(false) } }} title="长期权限版本历史" description="历史版本不可变；恢复会生成新的配置版本。恢复到更宽边界仍需一次性确认，且不影响当前运行期。" size="xl" footer={<><Button variant="outline" onClick={() => setHistoryOpen(false)}>关闭</Button><Button disabled={!selectedRevision || selectedContent === canonicalContent || !restoreConfirmed || busy} onClick={() => void restoreRevision()}>恢复为新版本</Button></>}>
...
function ManagedOrchestrationTab({ agent }: { agent: FullAgent }) {
  ...
  const save = async () => { if (!dirty) { setEditing(false); return }; if (!editor) { setError('编辑信息已过期。你的协作策略仍保留，请重新打开编辑器后再保存。'); return }; setBusy(true); setError(undefined); setConflict(undefined); try { const result = await saveConfig({ requestId: `save-orchestration-${agent.id}`, assetId: editor.asset.id, expectedOwner: { agentId: agent.id }, change: { kind: 'orchestration', value: proposedContent }, expectedBaseline: editor.baselineRef, baseContent: canonicalContent }); if (result.kind === 'saved' || result.kind === 'unchanged') commitCanonical(policy, proposedContent, result.kind === 'saved' ? '静态编排策略已保存到 Agent 配置' : '静态编排策略无变化'); else if (result.kind === 'baseline_changed') { setConflict(result); setError('orchestration.yaml 已被外部修改。Bandi 不会覆盖当前文件；你的修改仍保留，请比较三方内容后重新编辑。') } else { if (result.kind === 'save_failed' && result.recoveryRef) setRecoveryRef(result.recoveryRef); setError(result.diagnostics.map((item) => item.message).join('；')) } } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) } finally { setBusy(false) } }
  ...
  const recoverRevision = async () => { if (!editor || !recoveryRef) return; setBusy(true); try { const result = await recoverConfigRevision({ requestId: `recover-orchestration-${agent.id}`, assetId: editor.asset.id, recoveryRef }); if (result.kind === 'saved' || result.kind === 'unchanged') commitCanonical(policy, proposedContent, '静态编排策略配置版本已补记'); else setError(result.diagnostics.map((item) => item.message).join('；')) } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) } finally { setBusy(false) } }
...
function ManagedComponentReferences({ agent, componentKind }: { agent: FullAgent; componentKind: 'Hook' | 'Command' }) {
  ...
  const label = componentKind === 'Hook' ? '钩子引用' : '命令引用'
  ...
  const save = async () => {
    if (!dirty) { setEditing(false); return }
    if (!editor) { setError(`编辑信息已过期。你的${label}修改仍保留，请重新打开编辑器后再保存。`); return }
    ...
    try {
      ...
      if (result.kind === 'saved' || result.kind === 'unchanged') commitCanonical(references, proposedContent, result.kind === 'saved' ? `${label}已保存到 Agent 配置` : `${label}无变化`)
      else if (result.kind === 'baseline_changed') { setConflict(result); setError(`${fileName} 已被外部修改。Bandi 不会覆盖当前文件；你的修改仍保留，请比较三方内容后重新编辑。`) }
      else { if (result.kind === 'save_failed' && result.recoveryRef) setRecoveryRef(result.recoveryRef); setError(result.diagnostics.map((item) => item.message).join('；')) }
    } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) }
    finally { setBusy(false) }
  }
  ...
  const recoverRevision = async () => {
    ...
    try {
      ...
      if (result.kind === 'saved' || result.kind === 'unchanged') commitCanonical(references, proposedContent, `${label}配置版本已补记`)
  ...
  const restoreRevision = async () => {
    ...
    try {
      ...
      if (result.kind === 'saved' || result.kind === 'unchanged') { commitCanonical(parsed, selectedContent, result.kind === 'saved' ? `${label}已恢复为新的配置版本` : `${label}已是目标版本`); setHistoryOpen(false) }
  ...
  return <>
    <section className="panel p-5">
      <div className="flex items-center justify-between gap-3"><div><b>{label}</b><p className="mt-1 text-xs text-muted-foreground">独立保存到 config/{fileName}，只维护定义引用和非敏感参数，不{componentKind === 'Hook' ? '触发钩子' : '执行命令'}。</p></div>{editing ? <div className="flex gap-2"><Button variant="outline" size="sm" onClick={reset}>取消</Button><Button size="sm" disabled={!dirty || busy || Boolean(validationErrors.length)} onClick={save}>{busy ? '保存中…' : '保存'}</Button></div> : <Button variant="outline" size="sm" disabled={busy} onClick={beginEditing}>{busy ? '加载中…' : '编辑'}</Button>}</div>
      ...
      {error && <div role="alert" className="mt-3 text-sm text-danger"><p>{error}</p>{recoveryRef && <Button className="mt-3" variant="outline" size="sm" disabled={busy} onClick={recoverRevision}>补记配置版本</Button>}</div>}
...
function CollaborationMemoryTab({ agent }: { agent: FullAgent }) {
  ...
  return <><div className="space-y-5"><section className="panel overflow-hidden"><TabHeader title="协作与编排" description="只管理静态边界；有效委派仍取权限、组织、服务授权和此策略的交集。" editing={editing} onEdit={() => setEditing(true)} onCancel={reset} onSave={save} /><div className="p-5">{editing ? <div className="grid gap-4 md:grid-cols-2"><label className="flex items-center gap-3 rounded-lg border border-border p-4 text-sm font-medium"><input type="checkbox" checked={policy.enabled} onChange={(event) => setPolicy((value) => ({ ...value, enabled: event.target.checked }))} />允许委派（仍受其他边界约束）</label><Labeled label="最大委派深度"><input type="number" min={0} className="h-10 w-full px-3" value={policy.maxDelegationDepth} onChange={(event) => setPolicy((value) => ({ ...value, maxDelegationDepth: Number(event.target.value) }))} /></Labeled><Labeled label="允许 Agent"><select multiple className="min-h-32 w-full p-2" value={policy.allowedAgentIds} onChange={(event) => setPolicy((value) => ({ ...value, allowedAgentIds: Array.from(event.target.selectedOptions, (option) => option.value) }))}>{state.agents.filter((item) => item.id !== agent.id && item.status === 'active').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Labeled><Labeled label="允许岗位"><select multiple className="min-h-32 w-full p-2" value={policy.allowedRoleIds} onChange={(event) => setPolicy((value) => ({ ...value, allowedRoleIds: Array.from(event.target.selectedOptions, (option) => option.value) }))}>{state.roles.filter((item) => item.companyId === agent.companyId && item.status === 'active').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Labeled><Labeled label="允许部门"><select multiple className="min-h-32 w-full p-2" value={policy.allowedDepartmentIds} onChange={(event) => setPolicy((value) => ({ ...value, allowedDepartmentIds: Array.from(event.target.selectedOptions, (option) => option.value) }))}>{state.departments.filter((item) => item.companyId === agent.companyId).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Labeled><div className="space-y-2 rounded-lg border border-border p-3">{([['requireWorkspaceBinding', '必须有关联工作区配置'], ['requireSopMatch', '必须匹配 SOP'], ['requireServiceGrantForCrossDepartment', '跨部门必须有服务授权']] as const).map(([key, label]) => <label key={key} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={policy[key]} onChange={(event) => setPolicy((value) => ({ ...value, [key]: event.target.checked }))} />{label}</label>)}</div><Labeled label="升级目标"><select className="h-10 w-full px-3" value={policy.escalationAgentId ?? ''} onChange={(event) => setPolicy((value) => ({ ...value, escalationAgentId: event.target.value || undefined }))}><option value="">未设置</option>{state.agents.filter((item) => item.id !== agent.id && item.status === 'active').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Labeled><ListEditor label="升级条件" values={policy.escalationConditions} onChange={(value) => setPolicy((item) => ({ ...item, escalationConditions: value }))} /><ListEditor label="禁止事项" values={policy.prohibitions} onChange={(value) => setPolicy((item) => ({ ...item, prohibitions: value }))} /><ComponentReferenceEditor title="钩子引用" kind="Hook" references={hookRefs} assets={state.assets} onChange={setHookRefs} /><ComponentReferenceEditor title="命令引用" kind="Command" references={commandRefs} assets={state.assets} onChange={setCommandRefs} /></div> : <><div className="label">协作策略</div><h2 className="mt-2 text-lg font-semibold">长期协作与委派边界</h2><div className="mt-5"><FieldRow label="委派状态">{agent.orchestrationPolicy.enabled ? '允许（受其他边界约束）' : '禁止'}</FieldRow><FieldRow label="最大深度">{agent.orchestrationPolicy.maxDelegationDepth}</FieldRow><FieldRow label="允许 Agent">{agentNames.join('、') || '未授权任何 Agent'}</FieldRow><FieldRow label="允许岗位">{roleNames.join('、') || '未授权任何岗位'}</FieldRow><FieldRow label="允许部门">{departmentNames.join('、') || '未授权任何部门'}</FieldRow><FieldRow label="必需条件">{[agent.orchestrationPolicy.requireWorkspaceBinding && '需要关联工作区配置', agent.orchestrationPolicy.requireSopMatch && '需要 SOP 匹配', agent.orchestrationPolicy.requireServiceGrantForCrossDepartment && '跨部门需要服务授权'].filter(Boolean).join('；') || '无附加条件'}</FieldRow><FieldRow label="升级目标">{state.agents.find((item) => item.id === agent.orchestrationPolicy.escalationAgentId)?.name ?? agent.orchestrationPolicy.escalationAgentId ?? '未设置'}</FieldRow><FieldRow label="禁止事项">{agent.orchestrationPolicy.prohibitions.join('；')}</FieldRow></div></>}</div></section>{!editing && <div className="grid gap-5 lg:grid-cols-2"><ComponentReferences title="钩子引用" references={agent.hookRefs} assets={state.assets} kind="Hook" /><ComponentReferences title="命令引用" references={agent.commandRefs} assets={state.assets} kind="Command" /></div>}<MockBoundaryNote>存在引用不表示钩子已触发、命令已执行或当前会话已加载；Bandi 不接受命令行、工作目录、环境变量或可执行程序。</MockBoundaryNote></div>{unsavedDialog}</>
}
...
function WorkspacesTab({ agent }: { agent: FullAgent }) {
  ...
  const applyLoaded = (loaded: LoadEditorResult) => {
    ...
    if (!value) throw new Error('工作区配置内容格式不正确，或超出了 Agent 的基础边界')
  ...
  const commit = (value: WorkspaceBindingConfig, message: string) => {
    ...
    if (!next) throw new Error('工作区配置保存结果无法应用到当前 Agent')
  ...
  const save = async () => {
    ...
    if (!proposedContent) { setError('工作区配置无法生成，请修复配置后重试。'); return }
    setBusy(true); setError(undefined); setConflict(undefined)
    try {
      const result = editor
      ...
      if (result.kind === 'saved' || result.kind === 'unchanged') { commit(draft, result.kind === 'saved' ? '工作区配置已保存到 Agent 配置' : '工作区配置无变化'); reset() }
      else if (result.kind === 'baseline_changed') { setConflict(result); setError('工作区配置文件已在编辑期间被修改。请比较后基于磁盘当前内容重新编辑。') }
      else { const message = result.diagnostics.map((item) => item.message).join('；') || '工作区配置保存失败'; if (result.kind === 'save_failed' && result.recoveryRef) setRecoveryRef(result.recoveryRef); setError(message) }
    } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) }
    finally { setBusy(false) }
  }
  ...
  const selectRevision = async (revision: ConfigRevisionDto) => { setBusy(true); try { setSelectedRevision(revision); setSelectedContent(await readConfigRevisionContent(revision.id)); setRestoreConfirmed(false) } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) } finally { setBusy(false) } }
  const restoreRevision = async () => { if (!editor || !selectedRevision || !restoreConfirmed) return; const value = parseWorkspaceBindingConfig(selectedContent, agent); if (!value) { setError('目标历史版本不符合当前工作区配置边界，无法恢复。'); return } setBusy(true); setError(undefined); try { const result = await restoreConfigRevision({ requestId: `restore-workspace-binding-${agent.id}`, assetId: editor.asset.id, revisionId: selectedRevision.id, expectedBaseline: editor.baselineRef, baseContent: canonicalContent, confirmed: true }); if (result.kind === 'saved' || result.kind === 'unchanged') { commit(value, result.kind === 'saved' ? '工作区配置已恢复为新的配置版本' : '工作区配置已是目标版本'); setHistoryOpen(false); reset() } else if (result.kind === 'baseline_changed') { setHistoryOpen(false); setDraft(value); setConflict(result); setError('工作区配置文件已在恢复确认后被修改，请重新核对。') } else setError(result.diagnostics.map((item) => item.message).join('；') || '工作区配置恢复失败') } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) } finally { setBusy(false) } }
  ...
  const editorNode = draft ? <WorkspaceBindingEditor draft={draft} memoryRevision={memoryRevision} agentPolicy={agent.contextPolicy} agentOrchestration={agent.orchestrationPolicy} assets={state.assets} onInstructions={(instructions) => setDraft({ ...draft, instructions })} onToggle={toggle} onContext={updateContext} onOutputProfile={updateOutputProfile} onOutputParameters={updateOutputParameters} onOrchestration={updateOrchestration} onComponentRefs={updateComponentRefs} validationErrors={draftErrors} onCancel={reset} onSave={save} busy={busy} saveLabel={desktopManaged ? (busy ? '保存中…' : '保存') : '保存到当前页面'} /> : null
  return <><section className="panel overflow-hidden"><div className="flex items-center justify-between gap-3 border-b border-border p-4"><div><b>Agent 工作区专属配置</b><p className="mt-1 text-xs text-muted-foreground">配置保存在对应工作区的 config.yaml；正式记忆在记忆入口管理。{desktopManaged ? '桌面版会保存到受管 Agent 配置。' : '当前仅保存到当前页面。'}</p></div><Button variant="outline" size="sm" disabled={!availableWorkspace || Boolean(draft) || busy} onClick={openCreate}><Plus size={15} aria-hidden="true" />添加工作区配置</Button></div>{busy && <p role="status" className="border-b border-border px-5 py-3 text-sm text-muted-foreground">正在处理工作区配置…</p>}{error && <div role="alert" className="border-b border-danger/30 bg-danger/5 p-4 text-sm text-danger"><p>{error}</p>{recoveryRef && <Button className="mt-3" variant="outline" size="sm" disabled={busy} onClick={recoverRevision}>补记配置版本</Button>}</div>}{conflict && <div className="border-b border-border p-4"><div className="grid gap-3 lg:grid-cols-3" aria-label="工作区配置外部变化比较">{([{ label: '开始编辑时', side: conflict.base }, { label: '磁盘当前内容', side: conflict.current }, { label: '你的拟议内容', side: conflict.proposed }] as const).map(({ label, side }) => <section key={label} className="min-w-0 rounded-lg border border-border p-3"><b className="text-xs">{label}</b><pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap break-words text-xs leading-5">{side.content}</pre></section>)}</div><div className="mt-3 flex justify-end"><Button variant="outline" size="sm" disabled={busy} onClick={reloadConflict}>基于当前内容重新编辑</Button></div></div>}<div className="divide-y divide-border">{agent.workspaceBindings.map((binding) => { const workspace = state.workspaces.find((item) => item.id === binding.workspaceId); const isEditing = draft?.workspaceId === binding.workspaceId; return <div key={binding.workspaceId} className="p-5">{isEditing ? editorNode : <div className="grid gap-4 md:grid-cols-[1fr_1fr_auto]"><div>{workspace ? <Link to={`/workspaces/${binding.workspaceId}?tab=agents`} className="font-semibold hover:underline">{workspace.name}</Link> : <b>{binding.workspaceId}</b>}<p className="mt-1 text-xs text-muted-foreground">{workspace ? `正式项目记忆 ${binding.memoryRevision || '未设置'}` : '工作区索引缺失 · 配置仍保留'}</p></div><div className="text-sm text-muted-foreground">规则 {binding.ruleIds.length} · 技能 {binding.skillIds.length} · MCP {binding.mcpIds.length}<p className="mt-1">上下文：{binding.contextPolicy ? `覆盖 ${Object.keys(binding.contextPolicy).length} 项` : '继承'} · 输出格式：{binding.outputProfileId ? '显式' : '继承'}</p><p className="mt-1 line-clamp-2">{binding.instructions || '未设置专属主指令'}</p></div><div className="flex flex-wrap items-center gap-2"><StatusBadge tone={!workspace || !binding.ruleIds.length ? 'warning' : 'success'}>{!workspace ? '索引缺失' : binding.ruleIds.length ? '配置完整' : '尚未配置规则'}</StatusBadge><Button variant="ghost" size="sm" disabled={Boolean(draft) || busy} onClick={() => openHistory(binding.workspaceId)}><History size={14} aria-hidden="true" />历史</Button><Button variant="outline" size="sm" disabled={Boolean(draft) || busy} onClick={() => edit(binding)}>编辑</Button></div></div>}</div>})}{draft && !canonicalBinding && <div className="p-5">{editorNode}</div>}{!agent.workspaceBindings.length && !draft && <div className="p-5 text-sm text-muted-foreground">尚未创建 Agent 工作区专属配置。</div>}</div></section><AppDialog open={historyOpen} onOpenChange={(open) => { setHistoryOpen(open); if (!open) { setSelectedRevision(undefined); setRestoreConfirmed(false); setEditingId(undefined); setEditor(undefined) } }} title="工作区配置版本历史" description="历史版本不可变；恢复会重新确认工作区索引和当前文件未被修改，并生成新的配置版本。" size="xl" footer={<><Button variant="outline" onClick={() => setHistoryOpen(false)}>关闭</Button><Button disabled={!selectedRevision || selectedContent === canonicalContent || !restoreConfirmed || busy} onClick={restoreRevision}>恢复为新版本</Button></>}>{revisions.length ? <div className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]"><div className="space-y-2" role="list" aria-label="工作区配置版本">{revisions.map((revision) => <button key={revision.id} type="button" onClick={() => selectRevision(revision)} className={`w-full rounded-lg border p-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${selectedRevision?.id === revision.id ? 'border-foreground bg-muted' : 'border-border hover:bg-muted/60'}`}><b className="block truncate text-sm">{revision.id}</b><small className="mt-1 block text-muted-foreground">{formatDisplayTimestamp(revision.savedAt)} · {revision.summary}</small></button>)}</div><div className="min-w-0"><div className="grid gap-3 sm:grid-cols-2"><section className="min-w-0 rounded-lg border border-border"><div className="border-b border-border bg-muted px-3 py-2 text-xs font-semibold">磁盘当前内容</div><pre className="max-h-80 overflow-auto whitespace-pre-wrap break-words p-3 text-xs leading-6">{canonicalContent}</pre></section><section className="min-w-0 rounded-lg border border-border"><div className="border-b border-border bg-muted px-3 py-2 text-xs font-semibold">{selectedRevision?.id ?? '选择历史版本'}</div><pre className="max-h-80 overflow-auto whitespace-pre-wrap break-words p-3 text-xs leading-6">{selectedContent}</pre></section></div>{selectedRevision && selectedContent !== canonicalContent && <label className="mt-4 flex items-start gap-3 text-sm"><input className="mt-1" type="checkbox" checked={restoreConfirmed} onChange={(event) => setRestoreConfirmed(event.target.checked)} /><span>我已核对当前配置与目标历史内容，确认恢复并生成新版本。</span></label>}</div></div> : <p className="text-sm text-muted-foreground">当前工作区配置暂无配置版本。</p>}</AppDialog><AppDialog open={createOpen} onOpenChange={(open) => { setCreateOpen(open); if (!open) setSelectedWorkspaceId('') }} title="添加工作区配置" description="选择一个尚未关联的工作区。取消不会创建草稿或写入任何配置。" footer={<><Button variant="outline" onClick={() => setCreateOpen(false)}>取消</Button><Button disabled={!selectedWorkspaceId} onClick={confirmCreate}>确认选择</Button></>}><Labeled label="工作区"><select className="h-10 w-full px-3" value={selectedWorkspaceId} onChange={(event) => setSelectedWorkspaceId(event.target.value)}><option value="">请选择尚未关联的工作区</option>{availableWorkspaces.map((workspace) => <option key={workspace.id} value={workspace.id}>{workspace.name}（{workspace.id}）</option>)}</select></Labeled></AppDialog>{unsavedDialog}</>
}
function WorkspaceBindingEditor({ draft, memoryRevision, agentPolicy, agentOrchestration, assets, onInstructions, onToggle, onContext, onOutputProfile, onOutputParameters, onOrchestration, onComponentRefs, validationErrors, onCancel, onSave, busy, saveLabel }: { draft: WorkspaceBindingConfig; memoryRevision: string; agentPolicy: ContextPolicy; agentOrchestration: FullAgent['orchestrationPolicy']; assets: ReturnType<typeof useApp>['state']['assets']; onInstructions: (value: string) => void; onToggle: (field: 'ruleIds' | 'skillIds' | 'mcpIds', id: string) => void; onContext: (value?: ContextPolicyOverride) => void; onOutputProfile: (value?: string) => void; onOutputParameters: (value: ParameterBinding[]) => void; onOrchestration: (value?: WorkspaceBindingConfig['orchestrationPolicy']) => void; onComponentRefs: (field: 'hookRefs' | 'commandRefs', value: FullAgent['hookRefs']) => void; validationErrors: string[]; onCancel: () => void; onSave: () => void; busy: boolean; saveLabel: string }) {
  const groups = [{ label: '规则', field: 'ruleIds' as const, items: assets.filter((item) => item.kind === 'Rules') }, { label: '技能', field: 'skillIds' as const, items: assets.filter((item) => item.kind === 'Skill' && item.skill?.installation.status !== 'available') }, { label: 'MCP', field: 'mcpIds' as const, items: assets.filter((item) => item.kind === 'MCP') }]
