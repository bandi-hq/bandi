...
export function AiClientManagementSection() {
  ...
  return <><section className="panel overflow-hidden"><div className="flex flex-wrap items-end justify-between gap-4 border-b border-border px-5 py-4"><div><b>AI 编程工具</b><p className="mt-1 text-xs text-muted-foreground">当前方案只记录已加入的工具，不表示工具已安装、已探测或可交接。</p></div><div className="flex flex-wrap items-end gap-2"><label className="text-xs font-medium">当前配置方案<select aria-label="当前配置方案" className="mt-1 block h-9 min-w-40 px-3 text-sm" value={environment?.id ?? ''} onChange={(event) => dispatch({ type: 'SELECT_CONFIGURATION_ENVIRONMENT', environmentId: event.target.value })}>{state.configurationEnvironments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><Button size="sm" onClick={() => setCustomOpen(true)}><Plus size={15} />添加工具</Button></div></div><div className="border-b border-warning/20 bg-warning/8 px-5 py-3 text-xs text-warning">当前未探测本机工具；配置方案与加入状态仅在当前页面有效。</div><div className="divide-y divide-border">{state.aiClients.map((client) => { const registered = environment?.clientIds.includes(client.id) ?? false; const handoff = supportsWorkspaceHandoff(client); return <div key={client.id} className="grid gap-4 px-5 py-4 sm:grid-cols-[minmax(180px,1fr)_minmax(180px,1fr)_auto] sm:items-center"><div className="flex items-center gap-3"><span className="grid size-10 shrink-0 place-items-center"><AiClientIcon client={client} size={40} tile /></span><div><b className="block">{client.name}</b><span className="text-xs text-muted-foreground">{handoff ? '支持结构化终端交接 · 未探测安装状态' : '仅配置 · 尚未定义启动适配'}</span></div></div><div className="text-xs leading-5 text-muted-foreground"><div>{client.description}</div><div>{registered ? `已加入“${environment?.name}”` : `未加入“${environment?.name}”`}</div></div><div className="flex flex-wrap justify-end gap-2"><Button variant="outline" size="sm" onClick={() => environment && dispatch({ type: 'SET_ENVIRONMENT_CLIENT_REGISTRATION', environmentId: environment.id, clientId: client.id, registered: !registered })}>{registered ? '移出' : '加入当前方案'}</Button></div></div> })}</div><CustomClientDialog open={customOpen} onOpenChange={setCustomOpen} /></section></>
}
