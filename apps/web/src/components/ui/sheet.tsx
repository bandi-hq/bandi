...
export function Sheet({ open, onOpenChange, title, description, children, footer, wide, side = 'right', navigation }: Props) {
  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}><Dialog.Portal>
    ...
    <Dialog.Content onCloseAutoFocus={(event) => { if (!triggerRef.current?.isConnected) return; event.preventDefault(); triggerRef.current.focus(); triggerRef.current = null }} data-sheet-side={side} className={cn('fixed inset-y-0 z-50 flex w-full flex-col bg-background shadow-2xl outline-none', side === 'right' ? 'right-0 max-w-xl border-l border-border' : 'left-0 max-w-sm border-r border-border', wide && side === 'right' && 'max-w-3xl', navigation && 'max-w-[280px]')}>
