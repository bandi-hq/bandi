...
export function GlobalSheets() {
  ...
  if (dialog.kind === 'remove-workspace-index') {
    ...
    const remove = async () => { if (desktop) { try { await removeWorkspace(target.id) } catch { dispatch({ type: 'SHOW_NOTICE', notice: { tone: 'error', title: '无法移除工作区登记', description: '工作区登记没有变化。请检查本地服务后重试。' } }); return } } dispatch({ type: 'REMOVE_WORKSPACE_INDEX', workspaceId: target.id }) }
  ...
  if (dialog.kind === 'backup-restore') {
    ...
    return <AppDialog open onOpenChange={(open) => { if (!open) close() }} title="模拟范围恢复" description={`第 ${restoreStep} 步，共 3 步 · ${snapshot.id}`} size="lg" footer={<>{restoreStep > 1 && <Button variant="outline" onClick={() => setRestoreStep((step) => Math.max(1, step - 1) as 1 | 2 | 3)}>上一步</Button>}<Button variant="outline" onClick={close}>取消</Button>{restoreStep < 3 ? <Button disabled={restoreStep === 2 && !preview} onClick={() => setRestoreStep((step) => Math.min(3, step + 1) as 1 | 2 | 3)}>下一步</Button> : <Button variant="danger" disabled={!understood || !preview} onClick={restore}>确认模拟恢复</Button>}</>}>
      {restoreStep === 1 && <><InfoRow label="快照时间">{formatDisplayTimestamp(snapshot.createdAt)}</InfoRow><InfoRow label="快照范围">{describeBackupScope(snapshot.scope, state)}</InfoRow><label className="mt-5 block text-sm font-medium">恢复层级<select className="mt-2 h-10 w-full px-3" value={restoreScope.kind} onChange={(event) => chooseKind(event.target.value as BackupScope['kind'])}><option value="all">全部配置</option><option value="company">公司</option><option value="agent">Agent</option><option value="files">指定文件</option></select></label></>}
...
function WorkspaceResponsibilityDialog({ workspace, close }: { workspace: ReturnType<typeof useApp>['state']['workspaces'][number]; close: () => void }) {
  const { state, dispatch } = useApp(); const [companyId, setCompanyId] = useState(workspace.companyId ?? ''); const [primary, setPrimary] = useState(workspace.primaryDepartmentId ?? ''); const [lead, setLead] = useState(workspace.projectLeadAgentId ?? ''); const [collaborators, setCollaborators] = useState(workspace.collaboratorDepartmentIds); const [understood, setUnderstood] = useState(false); const [saving, setSaving] = useState(false); const [error, setError] = useState<UserFacingError>(); const desktop = isDesktopRuntime(); const companyDepartments = state.departments.filter((item) => item.companyId === companyId); const eligibleLeads = state.agents.filter((item) => item.companyId === companyId && item.status === 'active'); const invalidLead = Boolean(lead && !eligibleLeads.some((item) => item.id === lead)); const highImpact = companyId !== (workspace.companyId ?? '')
  ...
  return <AppDialog open onOpenChange={(open) => { if (!open) close() }} title="项目责任与组织关系" description={workspace.name} size="lg" footer={<><Button variant="outline" onClick={close}>取消</Button><Button disabled={saving || Boolean(companyId && !primary) || invalidLead || (highImpact && !understood)} onClick={save}>{saving ? '正在保存…' : desktop ? '保存关系' : '保存演示关系'}</Button></>}>
    ...
    {highImpact && <div className="mt-5 rounded-lg border border-warning/30 bg-warning/8 p-4 text-sm"><b>更换所属公司是高影响变更</b><p className="mt-2 text-muted-foreground">更换后会清空主责部门、默认负责人和协作部门，需重新选择并核对共享引用、服务授权和记忆可见性；不会删除目录、历史、Agent 配置或工作区专属配置。</p><label className="mt-3 flex gap-2"><input type="checkbox" checked={understood} onChange={(e) => setUnderstood(e.target.checked)} />我已了解影响并完成核对</label></div>}
