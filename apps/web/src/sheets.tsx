...
function buildCollaborationPlanningPrompt(goal: string, participants: string, boundaries: string): string {
  return [
    ...
    '',
    '如确有需要，请按最小治理顺序建议方案：先建立 Company、Department 与岗位，再创建或导入董事长助理及员工 Agent，随后回填董事长助理、部门主管等治理关系，最后才配置 WorkspaceBinding、SOP、长期权限与服务授权。一次性工作不要直接转成长期开岗或 Agent；组织身份、岗位和关系不自动授予权限。',
...
export function GlobalSheets() {
  ...
  if (dialog.kind === 'conflict') {
    ...
    return <AppDialog open onOpenChange={(open) => { if (!open) close() }} title="解决配置文件冲突" description="同一文本区域被同时修改，无法安全自动合并。" size="lg" footer={<><Button variant="outline" onClick={close}>取消保存</Button><Button disabled={!allResolved} onClick={() => done('2 处演示冲突已逐项解决 · 合并结果仅在当前页面有效')}>{allResolved ? '完成演示合并' : `请先解决 ${2 - Object.keys(conflicts).length} 处冲突`}</Button></>}>
...
function MemoryReviewDialog({ candidate, close }: { candidate: ReturnType<typeof useApp>['state']['memoryCandidates'][number]; close: () => void }) {
  ...
  return <AppDialog open onOpenChange={(open) => { if (!open) close() }} title={`审核正式记忆修改建议 ${candidate.id}`} description={desktop ? '审核关系、基线和写入结果由本地服务重新校验。' : '当前为浏览器演示，不会写入文件。'} size="xl" footer={<><Button variant="outline" disabled={loading || saving} onClick={() => desktop ? reviewDesktop('request_changes') : reviewDemo('要求修改')}>要求修改</Button><Button variant="outline" disabled={loading || saving} onClick={() => desktop ? reviewDesktop('reject') : reviewDemo('已驳回')}>驳回</Button><Button disabled={loading || saving || candidate.proposerAgentId === candidate.reviewerAgentId || Boolean(result && (result.kind === 'saved' || result.kind === 'review_recorded'))} onClick={() => desktop ? reviewDesktop('approve') : reviewDemo('已写入演示 Revision')}>{saving ? '正在处理…' : desktop ? '批准并写入正式记忆' : '批准并写入演示版本'}</Button></>}>
    {loading ? <p className="text-sm text-muted-foreground">正在读取正式记忆与审核起点…</p> : <><div className="grid gap-5 lg:grid-cols-[300px_1fr]"><div className="panel p-4"><InfoRow label="目标空间">{bundle?.space.scopeType ?? demoSpace?.scopeType ?? candidate.spaceId}</InfoRow><InfoRow label="所有者">{ownerLabel}</InfoRow><InfoRow label="归口">{state.agents.find((item) => item.id === bundle?.space.stewardAgentId)?.name ?? demoSpace?.steward ?? '—'}</InfoRow><InfoRow label="审核">{reviewer?.name ?? candidate.reviewerAgentId}</InfoRow><InfoRow label="提议者">{proposer?.name ?? candidate.proposerAgentId}</InfoRow><InfoRow label="状态"><StatusBadge tone={toneForStatus(status)}>{status}</StatusBadge></InfoRow>{result?.kind === 'saved' && <InfoRow label="正式版本">{result.revision.id}</InfoRow>}</div><div className="grid grid-cols-2 overflow-hidden rounded-lg border border-border max-sm:grid-cols-1"><div><div className="bg-muted px-4 py-2 text-xs font-semibold">当前正式内容</div><pre className="max-h-80 overflow-auto whitespace-pre-wrap break-words p-4 text-sm text-muted-foreground">{current || '（空）'}</pre></div><div className="border-l border-border max-sm:border-l-0 max-sm:border-t"><div className="bg-primary/8 px-4 py-2 text-xs font-semibold">建议写回</div><pre className="max-h-80 overflow-auto whitespace-pre-wrap break-words p-4 text-sm">{proposed}</pre></div></div></div>{candidate.proposerAgentId !== candidate.reviewerAgentId ? <div className="mt-5 flex items-center gap-2 text-sm text-success"><Check size={17} aria-hidden="true" />提议者与审核者已分离。</div> : <div className="mt-5 text-sm text-danger">提议者不能自审，请先调整审核者。</div>}{result?.kind === 'baseline_changed' && <div className="mt-5 rounded-lg border border-warning/30 bg-warning/8 p-4 text-sm text-warning">正式内容已在审核期间变化，请关闭后基于当前内容重新创建候选。</div>}{result?.kind === 'revision_pending' && <div className="mt-5 rounded-lg border border-warning/30 bg-warning/8 p-4 text-sm text-warning"><p>文件已写入，但记忆版本尚待补记。请勿重复批准或重复写入。</p><Button className="mt-3" variant="outline" size="sm" disabled={saving} onClick={recoverRevision}>{saving ? '正在补记…' : '补记记忆版本'}</Button></div>}{diagnostic && <p role="alert" className="mt-4 text-sm text-danger">{diagnostic}</p>}{error && <p role="alert" className="mt-4 text-sm text-danger">{error}</p>}</>}
  </AppDialog>
}
...
function ClientGuideDialog({ client, workspace, agent, planning = false, close }: { client: ReturnType<typeof useApp>['state']['aiClients'][number]; workspace?: ReturnType<typeof useApp>['state']['workspaces'][number]; agent?: ReturnType<typeof useApp>['state']['agents'][number]; planning?: boolean; close: () => void }) {
  ...
  const launch = async () => {
    ...
    try {
      ...
      close()
      notify('success', '系统已接受目录打开请求', `已请求在 ${terminal} 中打开工作区目录；请在自己的终端中启动 ${client.name}。`)
    } catch {
  ...
  return <AppDialog open onOpenChange={(open) => { if (!open) close() }} title={title} description={description} size="md" footer={supportsHandoff ? desktop ? <><Button variant="outline" onClick={close}>取消</Button><Button disabled={!workspace || opening} onClick={launch}>{opening ? '正在请求打开…' : `在 ${terminal} 中打开目录`}</Button></> : <Button onClick={close}>关闭</Button> : undefined}>
    {supportsHandoff ? <>
      ...
      <div className="mt-4 flex gap-2 rounded-md bg-muted p-3 text-sm text-muted-foreground"><Info size={18} className="shrink-0" aria-hidden="true" /><span>Bandi 只提交客户端、Adapter、工作区、终端和交接意图；后端从 Workspace Registry 重取目录，不读取终端内容，也不跟踪后续会话。</span></div>
...
function WorkspaceResponsibilityDialog({ workspace, close }: { workspace: ReturnType<typeof useApp>['state']['workspaces'][number]; close: () => void }) {
  const { state, dispatch } = useApp(); const [companyId, setCompanyId] = useState(workspace.companyId ?? ''); const [primary, setPrimary] = useState(workspace.primaryDepartmentId ?? ''); const [lead, setLead] = useState(workspace.projectLeadAgentId ?? ''); const [collaborators, setCollaborators] = useState(workspace.collaboratorDepartmentIds); const [understood, setUnderstood] = useState(false); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const desktop = isDesktopRuntime(); const companyDepartments = state.departments.filter((item) => item.companyId === companyId); const highImpact = companyId !== (workspace.companyId ?? '')
...
function OrganizationDialog({ dialog, close }: { dialog: Extract<NonNullable<ReturnType<typeof useApp>['state']['dialog']>, { kind: 'organization' }>; close: () => void }) {
  ...
  const [name, setName] = useState(currentCompany?.name ?? currentDepartment?.name ?? ''); const [companyId, setCompanyId] = useState(currentDepartment?.companyId ?? state.companies[0]?.id ?? ''); const [parentId, setParentId] = useState(currentDepartment?.parentDepartmentId ?? ''); const [mission, setMission] = useState(currentCompany?.mission ?? currentDepartment?.mission ?? ''); const [assistantAgentId, setAssistantAgentId] = useState(currentCompany?.assistantAgentId ?? ''); const [managerAgentId, setManagerAgentId] = useState(currentDepartment?.managerAgentId ?? ''); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const desktop = isDesktopRuntime(); const duplicate = dialog.entity === 'company' ? state.companies.some((item) => item.id !== dialog.id && item.name === name.trim()) : state.departments.some((item) => item.id !== dialog.id && item.companyId === companyId && item.name === name.trim())
  const companyAgents = state.agents.filter((item) => item.companyId === currentCompany?.id)
  const departmentMembers = state.agents.filter((item) => currentDepartment?.memberAgentIds.includes(item.id) && item.companyId === currentDepartment.companyId)
